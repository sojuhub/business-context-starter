...
5. Follow `plugins/business-context-starter/references/BUSINESS_DISCOVERY.md`. Conduct the required progressive interview about the most-used work sites/services, their purpose and other business sources; confirm previously supplied answers instead of repeating them. Offer scoped work-profile tab/recent-history candidate discovery and have the owner confirm services before content reads. Discover available source tools and account labels before asking for another document. Use existing evidence for a short company preview, then propose one bounded private-source scope. Follow `plugins/business-context-starter/references/CONNECT_SOURCES.md`: reuse a sufficient existing connector, execute approved reads, and offer a separately approved provider only for a real gap. Continue after scope approval without per-record questions. A first document is optional; a blocked source must not block others. Repository setup is only "starter prepared", not completed onboarding.
...
8. Resolve material knowledge gaps across all seven business categories and get owner review. Retain knowledge, provenance, excerpts and necessary originals. Verify saved knowledge in a distinct fresh session through an actual draft; record file versions and observed reads. Run the default `check_onboarding.py` full check. Source reads alone are insufficient; missing interview, review, storage or fresh-session evidence keeps onboarding incomplete. Respect stop/defer and record what remains. Test unrelated existing work without claiming unrun checks passed.
