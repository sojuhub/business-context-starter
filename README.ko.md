...
```mermaid
flowchart LR
    A["01 · 업무 파악<br/>사용 서비스와 자료"] --> B["02 · 내용 검토<br/>사실·규칙·모르는 내용"]
...
1. **실제 업무부터 묻습니다.** 가장 자주 사용하는 사이트·서비스와 그곳에서 하는 일을 단계별로 확인합니다. 승인받은 업무용 프로필의 열린 탭·최근 방문 목록에서도 후보를 찾고, 사용자가 업무 서비스를 확인한 뒤 자료를 읽습니다.
2. **자료를 읽고 부족한 내용을 묻습니다.** 고객·상품/서비스·가격/조건·업무 흐름/담당자·정책·문체·사용 서비스를 근거와 함께 정리하고 사용자 확인을 받습니다.
3. **검토한 지식을 저장합니다.** 회사 지식·출처·핵심 발췌와 필요한 가격표·매뉴얼 원문을 보관하고 업무 초안을 만듭니다.
4. **새 세션에서 활용돼야 완료합니다.** 저장된 기록을 실제로 읽고 업무 초안에 반영하는지 검증합니다. 인터뷰·중요한 공백 해소·검토·저장·재사용 중 하나라도 빠지면 미완료입니다. 중단·보류 요청은 존중하고 남은 일을 기록합니다.

에이전트는 기존 지침과 정상 작동하는 연결을 재사용합니다. 로그인과 동의는 계정
소유자가 진행합니다. 사용할 수 없는 도구와 아직 확인하지 못한 자료 접근은 명확히 알립니다.
Composio는 기존 연결에 필요한 기능이 없을 때 제안하는 선택 사항입니다.
저장소 준비는 **스타터 준비 완료**이며, 합의한 자료 수집과 요약 검토가 남아 있으면
온보딩 완료로 표시하지 않습니다. [연결 실행과 완료 검사](plugins/business-context-starter/references/CONNECT_SOURCES.md)는
기본 실행에서 전체 완료 기준과 저장 파일의 내용을 검사합니다. 실제 계정 접근·내용의 정확성을 독립적으로 입증하거나 호스트 에이전트의 행동 자체를 강제하는 기능은 아닙니다.
